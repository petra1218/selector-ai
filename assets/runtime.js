(function () {
  ...
  if (window[RUNTIME_SENTINEL] || document.querySelector(".ai-editor-root")) {
    return;
  }

  window[RUNTIME_SENTINEL] = true;
  ...
  function resetBootstrapFlag() {
    try {
      delete window[RUNTIME_SENTINEL];
    } catch (_) {
      window[RUNTIME_SENTINEL] = false;
    }
  }
  ...
  function showHover(el) {
    if (!hoverBox || !el || isEditorElement(el) || selectedElements.includes(el)) {
  ...
  function makeDraggable(panel, handle) {
    ...
    handle.addEventListener("mousedown", (e) => {
      ...
      st = r.top;
      const move = (event) => {
        panel.style.left = sl + event.clientX - sx + "px";
        panel.style.top = st + event.clientY - sy + "px";
